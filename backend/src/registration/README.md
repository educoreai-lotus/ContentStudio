...
2. **Registration Request**: POSTs to `${COORDINATOR_URL}/register` with service metadata (no authentication required)
...
- `400 Bad Request` → Check that serviceName, version, and endpoint are valid
- `401 Unauthorized` → May indicate Railway API Gateway authentication (not from Coordinator itself)
